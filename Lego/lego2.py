...
class LEGO:
    def __init__(self, **kwargs):
        ...
        # 1st conv layer
        self.w1 = tf.Variable(
            tf.truncated_normal([self.conv1_size, self.conv1_size, 3, self.conv1_filter_depth], stddev=0.1), name='w1')
        ...
        self.w2 = tf.Variable(tf.truncated_normal(
            [self.conv2_size, self.conv2_size, self.conv1_filter_depth, self.conv2_filter_depth], stddev=0.1),
            name='w2')
        ...
        if self.opt_conv > 1:
            self.w_opt2 = tf.Variable(tf.truncated_normal(
                [self.conv2_size, self.conv2_size, self.conv2_filter_depth * 2, self.conv2_filter_depth * 4],
                stddev=0.1),
                name='w_opt2')
    ...
    def _load_data_if_not(self, normalize_func):
        if self.train_data is None and self.train_label is None:
            self.train_data, self.train_label \
                = self._load_data(self.input, self.label, random_rotate=self.random_rotate,
